...
local RAZE_PURPLE = {119, 57, 185, 255}    -- color for racing HUD above map
local RAZE_POWDER_BLUE = {221, 219, 247, 255}
local RAZE_DARK_PURPLE = {66, 35, 157, 255}
local RAZE_LIGHT_PURPLE = {188, 86, 214, 255}
local RAZE_POWDER_BLUE = {152, 194, 246, 255}
local RAZE_WHITE = {221, 219, 247, 255}
-- State variables
local raceState = {
    cP = 1,
    index = 0 ,
    ...
    while raceState.index == 0 do
        ...
        for index, race in pairs(races) do
            if race.isEnabled then
                -- Draw map marker
                DrawMarker(1, race.start.x, race.start.y, race.start.z - 1, 0, 0, 0, 0, 0, 0, 3.0001, 3.0001, 1.5001, 157, 74, 208, 255, 0, 0, 0,0)
                ...
                if GetDistanceBetweenCoords( race.start.x, race.start.y, race.start.z, GetEntityCoords(player)) < DRAW_TEXT_DISTANCE then
                    -- Draw race name
                    Draw3DText(race.start.x, race.start.y, race.start.z-0.600, race.title, RAZE_POWDER_BLUE, 4, 0.3, 0.3)
                end
                        ...
                        if raceScores ~= nil then
                            ...
                            for k, score in pairs(drawScores) do
                                ...
                                if (k > #raceScoreColors) then
                                    ...
                                    Draw3DText(race.start.x, race.start.y, race.start.z+zOffset, string.format("%s %.2fs (%s)", score.car, (score.time/1000.0), score.player), RAZE_POWDER_BLUE, 4, 0.13, 0.13)
                                    zOffset = zOffset - 0.300
                                else
                                    -- Draw score with color and larger text, decrement offset
                                    Draw3DText(race.start.x, race.start.y, race.start.z+zOffset, string.format("%s %.2fs (%s)", score.car, (score.time/1000.0), score.player), RAZE_POWDER_BLUE, 4, 0.22, 0.22)
...
AddEventHandler("raceCountdown", function()
    ...
    Citizen.CreateThread(function()
        ...
        while getcountdown() > 0 do
            -- Update HUD
            Citizen.Wait(1)
            DrawHudText(getcountdown(), RAZE_LIGHT_PURPLE,0.5,0.4,4.0,4.0)
...
AddEventHandler("raceRaceActive", function()
    ...
    Citizen.CreateThread(function()
        ...
        checkpoint = CreateCheckpoint(1, race.checkpoints[raceState.cP].x,  race.checkpoints[raceState.cP].y,  race.checkpoints[raceState.cP].z + CHECKPOINT_Z_OFFSET, race.checkpoints[raceState.cP].x,race.checkpoints[raceState.cP].y, race.checkpoints[raceState.cP].z, race.checkpointRadius, 119, 57, 185, math.ceil(255*race.checkpointTransparency), 0)
        ...
        while raceState.index ~= 0 do 
            ...
            DrawHudText(("%.3fs"):format((GetGameTimer() - raceState.startTime)/1000), RAZE_PURPLE, 0.015, 0.725, 0.7, 0.7)
            DrawHudText(string.format("Checkpoint %i / %i (%d m)", raceState.cP, #race.checkpoints, checkpointDist), RAZE_WHITE, 0.015, 0.765, 0.5, 0.5)
...
function Draw3DText(x,y,z,textInput,colour,fontId,scaleX,scaleY)
    ...
    local colourr,colourg,colourb,coloura = table.unpack(RAZE_WHITE)
